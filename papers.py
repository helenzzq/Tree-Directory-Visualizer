...
class PaperTree(TMTree):
    ...
    _authors:
        The author(s) of this paper
    _doi:
        Digital Object Identifier of paper.
    ...
    def get_separator(self) -> str:
        """Return the file separator for this paper tree"""
        return '/'
...
def _load_papers_to_dict(by_year: bool = True) -> Dict:
    ...
    with open(DATA_FILE, newline='') as csvfile:
        ...
        for row in reader:
            ...
            if by_year:
                category.insert(0, year)
            i = 0
            insert_dict = nested_dictionary
            while i < len(category) and category[i] in insert_dict:
                insert_dict = insert_dict[category[i]]
                i += 1

            for a in range(i, len(category)):
                insert_dict[category[a]] = {}
                insert_dict = insert_dict[category[a]]
            if 'paper' not in insert_dict:
                insert_dict['paper'] = []
            insert_dict['paper'].append(row)
...
def _build_tree_from_dict(nested_dict: Dict) -> List[PaperTree]:
    ...
    """
    tree_list = []
    for cat in nested_dict.keys():
        if cat == 'paper':
            for item in nested_dict[cat]:
                paper_tree = PaperTree(item['Title'], [], item['Author'],
                                       item['Url'], int(item['Citations']))
                tree_list.append(paper_tree)

        else:
            inside_list = _build_tree_from_dict(nested_dict[cat])
            paper_tree = PaperTree(cat, inside_list)
            tree_list.append(paper_tree)
    return tree_list
